...
internal class BooksRoomDataSource @Inject constructor(
    private val booksDao: BooksDao,
) : BooksLocalDataSource {
    override fun getSavedBooks(): Flow<List<Book>> =
        booksDao.getSavedBooks().map { books -> books.map { it.toDomainModel() } }

    override fun findSavedBookById(bookId: String): Flow<Book?> =
        booksDao.findSavedBookById(bookId).map { it?.toDomainModel() }
